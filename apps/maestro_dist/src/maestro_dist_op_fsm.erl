...
op(N, W, Op, Key) ->
    ReqID = reqid(),
    maestro_dist_op_fsm_sup:start_write_fsm([ReqID, self(), Op, Key, N, W]),
    {ok, ReqID}.
...
prepare(timeout, SD0=#state{n=N, key=Key}) ->
    ...
    Preflist = riak_core_apl:get_apl(DocIdx, N, maestro_dist),
